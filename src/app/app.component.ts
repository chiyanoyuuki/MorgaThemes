...
@Component({
  ...
  animations: [
    trigger('moveAndRotate', [
      ...
      transition('start => end', [animate('300s ease-out')]),
      transition('end => start', [animate('300s ease-out')]),
    ]),
  ],
})
export class AppComponent implements OnInit {
  ...
  quadricites: any;
  quadr = {
    cardinaux: ['Bélier', 'Cancer', 'Balance', 'Capricorne'],
    fixes: ['Taureau', 'Lion', 'Scorpion', 'Verseau'],
    mutables: ['Gémeaux', 'Vierge', 'Sagittaire', 'Poissons'],
  };
  elements = {
  ...
  files: any = [
    { nom: 'Fichier Local' },
    {
      ...
      date: '01/01/1990',
      heure: '12:00',
      value: ['', 'exemple', '01', 'Janvier', '1990', '12', '00.txt'],
    },
  ];
  fileModel = 'Nouveau Theme';
  filesOpen = false;

  validmdp: any = ['toukoutou', 'trading', 'genki', 'elaine'];

  constructor(
    private http: HttpClient,
    private fileService: AutomationServiceService,
  ) {}
  ...
  getFiles(show: boolean = true) {
    ...
    else if (this.mdp.toLowerCase() == 'trading')
      this.files = [{ nom: 'Fichier Local' }];
    this.fileService
      .getFiles(isDevMode(), this.mdp.toLowerCase(), show)
      .subscribe(
        (data: any) => {
            ...
            if (nom != ' Exemple') {
              this.files.push({
                ...
                date:
                  ...
                  datas[4],
                heure:
                  datas[5] + ':' + datas[6].substring(0, datas[6].indexOf('.')),
                value: [
                  datas[0],
                  datas[1],
                  datas[2],
  ...
  fileChange(file: any) {
    if (file.nom == 'Thème Astral Cloé') {
      ...
      this.edit = true;
      this.VALUES = ['', '', 1, 'Janvier', 1990, 12, 0, ''];
      ...
      this.VALUES = [
        ...
        file.value[1][0].toUpperCase() +
          file.value[1].substring(1).toLowerCase(),
      ...
      if (
        i.prenom &&
        i.prenom.toLowerCase() ==
          v[1].toLowerCase() + ' ' + v[0].toLowerCase() &&
        i.date.includes('' + v[2]) &&
        i.date.includes(v[3].toLowerCase()) &&
        i.date.includes('' + v[4]) &&
        i.heure.includes('' + v[5]) &&
        i.heure.includes('' + v[6])
      ) {
  ...
  downloadFile() {
    this.loading = true;
    this.fileService.downloadFile(this.VALUES, isDevMode()).subscribe(
      (blob) => {
  ...
  readFile() {
    this.loading = true;
    this.http
      .get('assets/themes/astro.txt', { responseType: 'text' })
      .subscribe((text) => {
  ...
  readFile2() {
    this.loading = true;
    this.http
      .get('assets/themes/cloe.txt', { responseType: 'text' })
      .subscribe((text) => {
        this.fileContent = text;
        this.format();
      });
  ...
  readFile3() {
    this.loading = true;
    this.http
      .get('assets/themes/elaine.txt', { responseType: 'text' })
      .subscribe((text) => {
        this.fileContent = text;
        this.format();
      });
  ...
  readSigneGpt() {
    this.http
      .get('assets/signe.txt', { responseType: 'text' })
      .subscribe((text) => {
  ...
  format() {
    ...
    if (code.length == 1) code = this.fileContent.split('\n');
      ...
      if (s.includes(' en Maison ')) {
        if (s.includes('>Aspects')) {
          ...
          for (let i = 1; i < aspects.length; i++) {
            ...
            if (
              this.aspects.find(
                (as: any) => as.from == from && as.to == to && as.type == type,
              ) == undefined
            )
        ...
        if (nom == 'Nœud Nord')
          this.data.push({
            nom: 'Nœud Sud',
            signe: this.opposite(signe),
            maison: 'Pas de Maison',
          });
        ...
        s.includes('>Maison ') ||
        s.includes('>Milieu du Ciel ') ||
        s.includes('>Ascendant')
      ) {
        s = s.substring(s.indexOf('>') + 1);
        s = s.substring(0, s.indexOf('<'));
        ...
        if (
          !this.data.find(
            (d: any) => d.signe == signe && d.maison == nom && !d.nom,
          )
        )
          this.data.push({ signe: signe, maison: nom });
    ...
    this.stelliums = [];
    let dataplanetes = this.data.filter(
      (d: any) => d.nom && this.planetes.includes(d.nom),
    );
    dataplanetes.forEach((p: any) => {
      {
        ...
        if (planetes.length > 2) {
          ...
          for (let i = 0; i < planetes.length - 2; i++) {
            for (let j = i + 1; j < planetes.length - 1; j++) {
              for (let k = j + 1; k < planetes.length; k++) {
                let st = this.infos.stelliums.find(
                  (s: any) =>
                    s.noms.includes(planetes[i].nom) &&
                    s.noms.includes(planetes[j].nom) &&
                    s.noms.includes(planetes[k].nom) &&
                    s.signe == p.signe,
                );
                if (
                  stelliums.find(
                    (stel: any) =>
                      stel.noms == st.noms && stel.signe == st.signe,
                  ) == undefined
                )
                  stelliums.push(st);
              }
          ...
          stelliums.forEach((s: any) => {
            ...
            let plans = [
              this.getPlaneteFromDataByName(s.noms[0]),
              this.getPlaneteFromDataByName(s.noms[1]),
              this.getPlaneteFromDataByName(s.noms[2]),
            ];
            plans = plans.sort((a: any, b: any) => {
              return b.degres - a.degres;
            });
            ...
            if (ecart < 11) {
              if (
                this.stelliums.find(
                  (s2: any) =>
                    s2.noms.includes(s.noms[0]) &&
                    s2.noms.includes(s.noms[1]) &&
                    s2.noms.includes(s.noms[2]),
                ) == undefined
              )
    ...
    emispheres = {
      ...
      sud: emispheres.filter(
        (e: any) =>
          e.maison == 'Maison VII' ||
          e.maison == 'Maison VIII' ||
          e.maison == 'Maison IX' ||
          e.maison == 'Maison X' ||
          e.maison == 'Maison XI' ||
          e.maison == 'Maison XII',
      ).length,
      est: emispheres.filter(
        (e: any) =>
          e.maison == 'Maison X' ||
          e.maison == 'Maison XI' ||
          e.maison == 'Maison XII' ||
    ...
    this.general.emispherenord = this.infos.general.find(
      (g: any) => g.nord == emispheres.nord && g.sud == emispheres.sud,
    );
    this.general.emispherenord.data.forEach((d: any) => {
      d.data = this.setbold(d.data);
    });
    this.general.emisphereest = this.infos.general.find(
      (g: any) => g.est == emispheres.est && g.ouest == emispheres.ouest,
    );
    this.general.emisphereest.data.forEach((d: any) => {
      d.data = this.setbold(d.data);
    });
    ...
    this.addPolarite(
      elements.find((e: any) => e.nom == 'Lune').signe,
      'Lune',
      4,
    );
    this.addPolarite(
      elements.find((e: any) => e.nom == 'Soleil').signe,
      'Soleil',
      4,
    );
    let maitre = this.getMaitre(
      this.data.find((d: any) => d.maison == 'Maison I' && !d.nom).signe,
    );
    this.addPolarite(
      elements.find((e: any) => e.nom == maitre).signe,
      maitre,
      4,
    );
    this.addPolarite(
      elements.find((e: any) => e.nom == 'Mercure').signe,
      'Mercure',
      3,
    );
    this.addPolarite(
      elements.find((e: any) => e.nom == 'Vénus').signe,
      'Vénus',
      3,
    );
    this.addPolarite(
      elements.find((e: any) => e.nom == 'Mars').signe,
      'Mars',
      3,
    );
    this.addPolarite(
      elements.find((e: any) => e.nom == 'Jupiter').signe,
      'Jupiter',
      2,
    );
    this.addPolarite(
      elements.find((e: any) => e.nom == 'Saturne').signe,
      'Saturne',
      2,
    );
    this.addPolarite(
      elements.find((e: any) => e.nom == 'Uranus').signe,
      'Uranus',
      1,
    );
    this.addPolarite(
      elements.find((e: any) => e.nom == 'Neptune').signe,
      'Neptune',
      1,
    );
    this.addPolarite(
      elements.find((e: any) => e.nom == 'Pluton').signe,
      'Pluton',
      1,
    );

    this.general.eau = elements.filter((e: any) =>
      this.elements.eau.includes(e.signe),
    ).length;
    this.general.air = elements.filter((e: any) =>
      this.elements.air.includes(e.signe),
    ).length;
    this.general.feu = elements.filter((e: any) =>
      this.elements.feu.includes(e.signe),
    ).length;
    this.general.terre = elements.filter((e: any) =>
      this.elements.terre.includes(e.signe),
    ).length;

    let pol = this.infos.general.find(
      (i: any) => i.nom == 'eau' && i.points == this.polarites.eau,
    );
    this.modalites[0].data.push({
      nom: 'eau',
      points: pol.points,
      data: this.setbold(pol.data),
    });
    pol = this.infos.general.find(
      (i: any) => i.nom == 'air' && i.points == this.polarites.air,
    );
    this.modalites[0].data.push({
      nom: 'air',
      points: pol.points,
    ...
    pol = this.infos.general.find(
      (i: any) => i.nom == 'feu' && i.points == this.polarites.feu,
    );
    this.modalites[0].data.push({
      nom: 'feu',
      points: pol.points,
    ...
    pol = this.infos.general.find(
      (i: any) => i.nom == 'terre' && i.points == this.polarites.terre,
    );
    this.modalites[0].data.push({
      nom: 'terre',
      points: pol.points,
    ...
    this.modalites[0].data = this.modalites[0].data.sort((a: any, b: any) => {
      return b.points - a.points;
    });

    pol = this.infos.general.find(
      (i: any) => i.nom == 'cardinaux' && i.points == this.polarites.cardinaux,
    );
    this.modalites[1].data.push({
      nom: 'cardinaux',
      points: pol.points,
      data: this.setbold(pol.data),
    });
    pol = this.infos.general.find(
      (i: any) => i.nom == 'fixes' && i.points == this.polarites.fixes,
    );
    this.modalites[1].data.push({
      nom: 'fixes',
      points: pol.points,
    ...
    pol = this.infos.general.find(
      (i: any) => i.nom == 'mutables' && i.points == this.polarites.mutables,
    );
    this.modalites[1].data.push({
      nom: 'mutables',
      points: pol.points,
    ...
    this.modalites[1].data = this.modalites[1].data.sort((a: any, b: any) => {
      return b.points - a.points;
    });

    pol = this.infos.general.find(
      (i: any) => i.yin == this.polarites.eau + this.polarites.terre,
    );
    this.modalites[2].data.push({
      yin:
        this.modalites[0].data.find((m: any) => m.nom == 'terre').points +
        this.modalites[0].data.find((m: any) => m.nom == 'eau').points,
      yang: 0,
    ...
    this.domaines.forEach((dom: any) => {
      let domaine = this.infos.domaines.filter(
        (d: any) => d.domaine == dom.nom,
      );
      //Planetes
      let data = this.data.filter((d: any) => dom.planetes.includes(d.nom));
      data.forEach((d: any) => {
        let signe = domaine.find(
          (f: any) => f.planete == d.nom && f.signe == d.signe,
        );
        let maison = domaine.find(
          (f: any) => f.planete == d.nom && f.maison == d.maison,
        );
      ...
      data = this.data.filter(
        (d: any) => !d.nom && dom.maisons.includes(d.maison),
      );
      data.forEach((d: any) => {
        let signe = domaine.find(
          (f: any) => f.maison == d.maison && f.signe == d.signe,
        );
        dom.domaines.push(this.setbold(signe.data));
      });
      //Aspects
      data = this.aspects.filter(
        (a: any) =>
          dom.types.includes(a.type) &&
          ((dom.planetes.includes(a.from) && dom.dissonance.includes(a.to)) ||
            (dom.planetes.includes(a.to) && dom.dissonance.includes(a.from))),
      );
      data.forEach((d: any) => {
        let aspect = domaine.find(
          (f: any) =>
            (f.from == d.from && f.to == d.to && f.type == d.type) ||
            (f.from == d.to && f.to == d.from && f.type == d.type),
        );
      ...
      dom.exces.forEach((e: any) => {
        if (e == 'Eau/Yin') {
          ...
          if (eau > 5) {
            dom.domaines.push(
              this.setbold(
                this.infos.domaines.find((d: any) => d.exces == 'Eau/Yin').data,
              ),
            );
          }
        } else if (e == 'Capricorne') {
          let planetes = this.data.filter(
            (d: any) =>
              d.nom && this.planetes.includes(d.nom) && d.signe == 'Capricorne',
          ).length;
          let stellium = this.stelliums.find(
            (s: any) => s.signe == 'Capricorne',
          );
          if (planetes > 3 || stellium) {
            dom.domaines.push(
              this.setbold(
                this.infos.domaines.find((d: any) => d.exces == 'Capricorne')
                  .data,
              ),
            );
          }
  ...
  getGeneral() {
    ...
    else if (this.clicked2 == this.menu[0])
      retour = this.general.emispherenord.data;
    else if (this.clicked2 == this.menu[1])
      retour = this.general.emisphereest.data;
  ...
  init(first: boolean) {
    ...
    for (let i = 1; i < this.svg.length; i++) {
      ...
      if (s.includes('objet27"')) {
        ...
        y = 510 - y;
        let svg =
          '<image id="objet100" class="planete noeudSud" x="' +
          x +
          '" y="' +
          y +
          '" width="35" height="35" href="./assets/noeudsud.png"></image>';
        this.svg.push(svg);
      ...
      if (s.match(/.*<image.*/g)) {
        ...
        if (nom == 'objet100') {
          ...
          this.planetes.includes(nom2) ||
          this.asteroides.includes(nom2) ||
          nom == 'objet30' ||
          nom == 'objet31'
        ) {
          this.svg[i + 1] =
            ...
            nom +
            '-2" ' +
            this.svg[i + 1].substring(this.svg[i + 1].indexOf('x1='));
          this.svg[i + 2] =
            '<line id="' +
            nom +
            '-3" ' +
            this.svg[i + 2].substring(this.svg[i + 2].indexOf('x1='));
          this.svg[i + 3] =
            '<text id="' +
            nom +
            '-4" ' +
            this.svg[i + 3].substring(this.svg[i + 3].indexOf('x='));
          this.svg[i + 4] =
            '<text id="' +
            nom +
            '-5" ' +
            this.svg[i + 4].substring(this.svg[i + 4].indexOf('x='));
        }
      }
    ...
    this.svgs.forEach((svg: any) => {
      svg.active =
        this.data.find(
          (d: any) =>
            d.nom == svg.nom || d.signe == svg.nom || d.maison == svg.nom,
        ) != undefined;
      if (!svg.active) {
  ...
  getAspectscoords() {
    ...
    data = data.filter(
      (d: any) =>
        d.getAttribute('style') != 'stroke: #000000' &&
        d.getAttribute('id') == null &&
        d.getAttribute('class') == null,
    );
    /*let i = 0;
    data.forEach((d:any)=>{
      d.id = "line-"+i++;
  ...
  getPlaneteFromDataByName(name: any) {
    return this.data.find((planete: any) => planete.nom == name);
  }
  ...
  click(s: string) {
    ...
    this.focus = [];
    this.clicked = s;
    this.desc = this.setbold(
      this.infos.desc.find((d: any) => d.nom == s).infos,
    );
    this.type = 'Planetes';
    ...
    if (this.type == 'Asteroides' || this.type == 'Planetes') {
      ...
      data.forEach((d: any) => {
        ...
        if (s != 'Nœud Sud') {
          ...
          let x = this.infos.maisons.find(
            (m: any) => m.maison == d.maison && m.nom == d.nom,
          );
      ...
      aspects.forEach((a: any) => {
        let infos = this.infos.aspects.find(
          (i: any) => i.from == a.from && i.type == a.type && i.to == a.to,
        );
        infos.data.forEach((d: any) => (d.data = this.setbold(d.data)));
        this.focus.push({
          nom: infos.from + ' ' + infos.type + ' ' + infos.to,
          data: infos.data,
          type: 'Aspects',
        });
      });

      let domiciles = this.infos.domiciles.filter(
        (d: any) => d.nom == s && d.signe == ligne.signe,
      );
      domiciles.forEach((d: any) => {
        d.data.forEach((d: any) => (d.data = this.setbold(d.data)));
        this.focus.push({
          nom: d.nom + ', ' + d.type + ' en ' + d.signe,
          data: d.data,
          type: 'Dignités',
        });
      });
      ...
      data.forEach((d: any) => {
        ...
        if (!d.nom) {
          ...
          this.focus.push({
            nom: x.nom,
            data: x.data,
            type: this.planetes.includes(x.nom) ? 'Planetes' : 'Asteroides',
          });
        }
      });
